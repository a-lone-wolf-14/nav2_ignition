...
def generate_launch_description():
    ...
    # Ignition Gazebo Fortress launch
    ignition_launch = ExecuteProcess(
        cmd=[
            "ign",
            "gazebo",
            "-v",  # Enable verbose mode
            "-s",  # Add ROS 2 support plugin
            "libign_ros_init.so",
            "--render-engine", "ogre2",  # Specify rendering engine (optional)
            LaunchConfiguration("ignition_world"),
        ],
        output="screen",
    )
    ...
    return launch.LaunchDescription(
        [
            ...
            DeclareLaunchArgument(
                ...
                default_value=[
                    FindPackageShare("sam_bot_nav2_gz"),
                    "/worlds/your_world.sdf",
                ],
                description="Path to the Ignition Fortress world file",
            ),
            ignition_launch,  # Launch Ignition Fortress
            bringup,
            waiting_toolbox,
            waiting_navigation,
            waiting_success,
